...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-ink-200 bg-white shadow-[0_-3px_18px_rgba(15,23,42,0.08)]">
      <div className="mx-auto flex w-full max-w-[30rem] items-stretch justify-between px-1.5 pb-[env(safe-area-inset-bottom)] pt-2.5">
        {items.map(({ href, labelKey, Icon, primary }) => {
          ...
          if (primary) {
            return (
              <Link
                ...
                className="flex flex-1 flex-col items-center justify-end gap-1.5 pb-2.5"
              >
                <span className="relative -mt-6">
                  <span
                    ...
                    aria-hidden
                  />
                  <span className="relative grid size-[3.75rem] place-items-center rounded-[1.25rem] bg-blue-600 text-white shadow-[0_8px_22px_rgba(37,99,235,0.5)]">
          ...
          return (
            <Link
              ...
              href={href}
              aria-current={active ? "page" : undefined}
              className={`flex flex-1 flex-col items-center gap-1.5 py-2 text-[12px] font-semibold transition ${
